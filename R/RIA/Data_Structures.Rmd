...
|---|---|---|---|
|DELIMITED FILES| | read.table() ,read.csv()||
|XLS|xlsx,rodbc|read.xlsx()|Create worksheets in a workbook, read from an excel 2007 file i.e xlsx, which is a collection of zipped xmls|
|ZIPPED FILES||gzfile(),bzfile(),xzfile()||
|RDBMS|RODBC, DBI||Also helpful to read from ms excel, ms access|
|XML|||Useful for putting the semi-structured xml format into structured format, http://www.omegahat.net/RSXML/Tour.pdf|
|WEB SCRAPING|Rcurl and xml to read complex web data|url()- to make a web connection ,readLines() - to read text data, gsub() and grep() for pattern finding and replacement| http://www.programmingr.com/|
|HDFS - HIVE TABLES, PARQUET FILES|sparklyr,sparklyr +  DBI||https://spark.rstudio.com/|
|JSON|jsonlite, rjson|||
|SPSS, SAS, STATA|Hmisc, foreign (SPSS & SAS)|read.spss(),read.ssd, spss.get(),sas.get||
