...
|---|---|---|---|
|DELIMITED FILES| | read.table() ,read.csv()|Key arguments : read.table(file = ,header = ,sep = ,nrows = ,skip = ,colClasses = ,stringsAsFactors = ,na.strings = , nrow = )|
|XLS|xls||* Create worksheets in a workbook, read from an excel 2007 file i.e xlsx, which is a collection of zipped xmls|
|ZIPPED FILES||gzfile(),bzfile(),xzfile()||
|RDBMS|RODBC||Also helpful to read from ms excel, ms access; library(RODBC)
